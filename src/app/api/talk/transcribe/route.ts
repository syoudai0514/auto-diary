...
import { buildTalkTranscribePrompt } from '@/lib/prompt';
 ...
 * 通常の文字起こしと同様、音声はサーバー/Vercel 上に永続保存しない。
 * 音声データ・本文はログに一切出力しない。
 */
export async function POST(req: Request) {
  ...
  const limited = await rateLimitDistributed(`talk-transcribe:${userId}`, {
    // 長い話し合いはチャンク分割で複数回に分けて送られるため余裕を持たせる
    capacity: 6,
    refillPerSec: 6 / 60,
  });
  if (!limited.ok) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        {
          role: 'user',
          parts: [{ text: buildTalkTranscribePrompt() }, { inlineData: { mimeType, data: base64 } }],
        },
      ],
      config: {
        temperature: 0,
    ...
    return aiErrorResponse(
      { tag: 'talk-transcribe', code: 'transcription_failed', messageBase: '文字起こしに失敗しました' },
      err,
    );
  }
}
