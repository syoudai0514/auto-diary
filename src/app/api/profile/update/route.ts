...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error('[profile-update] failed:', status, reason || (err instanceof Error ? err.name : 'UnknownError'));
    return NextResponse.json(
      {
        error: 'update_failed',
        message: reason ? `プロフィールの更新に失敗しました（${reason}）` : 'プロフィールの更新に失敗しました。',
      },
      { status },
    );
  }
...
/** Gemini SDK のエラーメッセージ（API側の失敗理由。ユーザーのプロフィール本文は含まれない）。 */
